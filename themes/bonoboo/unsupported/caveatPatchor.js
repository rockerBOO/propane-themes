      ...
      if (this.actsLikeTextMessage()) {
        ...
        if (author.visible()) {
          ...
          if (this.bodyCell.select('strong').length === 0) {
            this.bodyCell.insert({top: '<strong style="color:#010101; float: right;"></strong>'});
            avatar = author.getAttribute('data-avatar') || 'http://asset1.37img.com/global/missing/avatar.png?r=3';
            author.insert({after: '<img alt="'+this.author()+'" width="24" height="24" align="top" style="box-shadow: 0 1px 2px #333; opacity: 1.0; border-radius:3px;" src="'+avatar+'">'});
          }
        }
      }
    }
    ...
    onEmbedDataLoaded: function(messageID, data) {
      ...
      if (data['item_type'] === 'image') {
        ...
        message.resize((function() {
          message.bodyCell.insert({bottom: '<div style="width:100%; margin-top:5px; padding-top: 5px; border-top:1px dotted #010101;"><a href="'+imageURL+'" class="image loading" target="_blank">' + '<img src="'+imageURL+'" onload="$dispatch(&quot;inlineImageLoaded&quot;, this)" onerror="$dispatch(&quot;inlineImageLoadFailed&quot;, this)" /></a></div>'});
        }).bind(this));
      }
    },
